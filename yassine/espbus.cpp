...
volatile unsigned long espbus::_cardTempHigh=0;
volatile unsigned long espbus::_cardTemp=0;
volatile unsigned long espbus::_lastespbus=0;
unsigned long espbus::_code=0;
volatile int espbus::_bitCount=0;	
int espbus::_espbusType=0;

espbus::espbus()
{
}

unsigned long espbus::getCode()
{
	return _code;
}

int espbus::getespbusType()
{
	return _espbusType;
}
...
void espbus::beginrs(int pinD0, int pinD1)
{
	_lastespbus = 0;
	_cardTempHigh = 0;
	_cardTemp = 0;
...
INTERRUPT_ATTR void espbus::ReadD0 ()
{
	...
	_lastespbus = millis();	// Keep track of last espbus bit received
}

INTERRUPT_ATTR void espbus::ReadD1()
{
...
unsigned long espbus::GetCardId (volatile unsigned long *codehigh, volatile unsigned long *codelow, char bitlength)
{
...
bool espbus::DoespbusConversion ()
{
	unsigned long cardID;
	unsigned long sysTick = millis();
	
	if ((sysTick - _lastespbus) > 25)								// if no more signal coming through after 25ms
	{
		if ((_bitCount==24) || (_bitCount==26) || (_bitCount==32) || (_bitCount==34) || (_bitCount==8) || (_bitCount==4)) 	// bitCount for keypress=4 or 8, espbus 26=24 or 26, espbus 34=32 or 34
		{
			...
			if (_bitCount==8)		// keypress espbus with integrity
			{
				// 8-bit espbus keyboard data, high nibble is the "NOT" of low nibble
				// eg if key 1 pressed, data=E1 in binary 11100001 , high nibble=1110 , low nibble = 0001 
				char highNibble = (_cardTemp & 0xf0) >>4;
				char lowNibble = (_cardTemp & 0x0f);
				_espbusType=_bitCount;					
				_bitCount=0;
				_cardTemp=0;
				_cardTempHigh=0;
				...
				// 4-bit espbus codes have no data integrity check so we just
				// read the LOW nibble.
...
char translateEnterEscapeKey(char originalKeyPress) {
	switch(originalKeyPress) {
	case 0x0b:        // 11 or * key
		return 0x0d;  // 13 or ASCII ENTER
